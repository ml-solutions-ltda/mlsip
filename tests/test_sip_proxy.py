...
async def test_proxy_subscribe(test_server, test_proxy, protocol, loop, from_details, to_details, close_order):
    ...
    class ServerDialplan(mlsip.BaseDialplan):
    ...
    class ProxyDialplan(mlsip.BaseDialplan):
    ...
    server_app = mlsip.Application(loop=loop, debug=True, dialplan=ServerDialplan())
    await test_server(server_app)

    proxy_app = mlsip.Application(loop=loop, debug=True, dialplan=ProxyDialplan())
