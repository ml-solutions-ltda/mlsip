...
async def run_call(peer, duration):
    call = await peer.invite(
        ...
        to_details=mlsip.Contact.from_header('sip:666@{}:{}'.format(
...
def main():
    ...
    if args.protocol == 'udp':
        loop.run_until_complete(start(app, mlsip.UDP, args.duration))
    elif args.protocol == 'tcp':
        loop.run_until_complete(start(app, mlsip.TCP, args.duration))
    elif args.protocol == 'ws':
        loop.run_until_complete(start(app, mlsip.WS, args.duration))
    else:
